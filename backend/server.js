...
const createUser = async (req, res) => {
  ...
  if (data != {}) {
    try {
      const { orcid, name, password, email } = data
      if (!name || !orcid || !password || !email) {
      ...
      sendMail(
        email,
        'Verify Your Account - ScholarChat',
        `
      ...
      res.status(201)
      res.send('User Created Successfully. Check email to Verify.')
    } catch (e) {
      // Throw error
      console.log('catch')
